#include "CppUnitTest.h"
#include "TestToString.h"

#include "Utils.h"
#include "MathLib/Types/Vec3.h"

using namespace Microsoft::VisualStudio::CppUnitTestFramework;
using ::MathLib::Vec3;
using namespace MathClasses;

namespace MathLibraryTests
{
	TEST_CLASS(Vec3Tests)
	{
	public:
		TEST_METHOD(DefaultConstructor)
		{
			Vec3 vec;
			Assert::AreEqual(0.f, vec.x);
			Assert::AreEqual(0.f, vec.y);
			Assert::AreEqual(0.f, vec.z);
		}

		TEST_METHOD(SpecializedConstructor)
		{
			Vec3 vec(1.f,2.f,3.f);
			Assert::AreEqual(1.f, vec.x);
			Assert::AreEqual(2.f, vec.y);
			Assert::AreEqual(3.f, vec.z);
		}

		TEST_METHOD(Magnitude)
		{
				Vec3 v3a(13.5f, -48.23f, 862);
				float mag3 = v3a.Magnitude();
				Assert::AreEqual(863.453735352f, mag3, MAX_FLOAT_DELTA);

				Vec3 v3b(0, 0, 0);
				mag3 = v3b.Magnitude();
				Assert::AreEqual(0.f, mag3, MAX_FLOAT_DELTA);
		}

        TEST_METHOD(Normalised)
        {
            Vec3 v3a(13.5f, -48.23f, 862);
            Assert::AreEqual(Vec3(0.0156349f, -0.0558571f, 0.998316f), v3a.Normalised());

            Vec3 v3b(0, 0, 0);
            Assert::AreEqual(Vec3(0, 0, 0), v3b.Normalised());
        }

		TEST_METHOD(Normalise)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			v3a.Normalise();
			Assert::AreEqual(Vec3(0.0156349f, -0.0558571f, 0.998316f), v3a);
			
			Vec3 v3b(0, 0, 0);
			v3b.Normalise();
			Assert::AreEqual(Vec3(0,0,0), v3b);
		}		
		
		TEST_METHOD(Dot)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3b(5, 3.99f, -12);
			float dot3 = v3a.Dot(v3b);

			Assert::AreEqual(-10468.9375f, dot3);
			
			
			Vec3 v3c(0, 0, 0);
			Vec3 v3d(5, 3.99f, -12);
			dot3 = v3c.Dot(v3d);

			Assert::AreEqual(0.0f, dot3);
		}

		TEST_METHOD(Cross)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3b(5, 3.99f, -12);
			Vec3 v3c = v3a.Cross(v3b);

			Assert::AreEqual(Vec3(-2860.62011719f, 4472.00000000f, 295.01498413f), v3c);
		}

		TEST_METHOD(Add)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3b(5, 3.99f, -12);
			Vec3 v3c = v3a + v3b;

			Assert::AreEqual(Vec3(18.5f, -44.24f, 850), v3c);
		}

		TEST_METHOD(Subtract)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3b(5, 3.99f, -12);
			Vec3 v3c = v3a - v3b;

			Assert::AreEqual(Vec3(8.5f, -52.22f, 874), v3c);
		}

		TEST_METHOD(ScalarMulVectorRight)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3c = v3a * 0.256f;

			Assert::AreEqual(Vec3(3.45600008965f, -12.3468809128f, 220.672012329f), v3c);
		}

		TEST_METHOD(ScalarMulVectorLeft)
		{
			Vec3 v3a(13.5f, -48.23f, 862);
			Vec3 v3c = 0.256f * v3a;

			Assert::AreEqual(Vec3(3.45600008965f, -12.3468809128f, 220.672012329f), v3c);
		}

		
		// TODO: implement tests for compound operators
	};

}

//namespace MathLibraryTests_OPTIONAL
//{
//	TEST_CLASS(Vec3OptionalTests)
//	{
//		TEST_METHOD(ScalarDivide)
//		{
//			Vec3 v3a(13.5f, -48.23f, 862);
//			Vec3 v3c = v3a / 0.256f;
//
//			Assert::AreEqual(Vec3(52.734371f, -188.398422f, 3367.187256f), v3c);
//		}
//		TEST_METHOD(Equality)
//		{
//			Vec3 v3a(13.5f, -48.23f, 862);
//			Vec3 v3b(13.5f, -48.23f, 862);
//
//			Assert::IsTrue(v3a == v3b);
//		}
//
//		TEST_METHOD(Inequality)
//		{
//			Vec3 v3a(13.5f, -48.23f, 862);
//			Vec3 v3b(0.0f, 0.0f, 0.0f);
//
//			Assert::IsTrue(v3a != v3b && !(v3a == v3b));
//		}
//		TEST_METHOD(SubscriptReference) {
//			const Vec3 v3a(13.5f, -48.23f, 862);
//
//			Assert::AreEqual(v3a[0], 13.5f);
//			Assert::AreEqual(v3a[1], -48.23f);
//			Assert::AreEqual(v3a[2], 862.f);
//		}
//		TEST_METHOD(Subscript) {
//			Vec3 v3a(0, 0, 0);
//			v3a[0] = 3;
//			Assert::AreEqual(v3a[0], 3.f);
//			v3a[1] = 4;
//			Assert::AreEqual(v3a[1], 4.f);
//			v3a[2] = 6;
//			Assert::AreEqual(v3a[2], 6.f);
//		}
//		TEST_METHOD(Cast)
//		{
//			Vec3 v3a(10, 2, 6);
//
//			float* p = (float*)v3a;
//			Assert::AreEqual(p[0], 10.f);
//			Assert::AreEqual(p[1], 2.f);
//			Assert::AreEqual(p[2], 6.f);
//
//			const float* q = (float*)v3a;
//			Assert::AreEqual(q[0], 10.f);
//			Assert::AreEqual(q[1], 2.f);
//			Assert::AreEqual(q[2], 6.f);
//		}
//	};
//}
